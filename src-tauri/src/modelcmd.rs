...
pub fn inference_yolov8(model: String, image_path: String) -> Result<Vec<Detection>, String> {
    ...
    let input_dims = session.inputs[0]
        .input_type
        .tensor_dimensions()
        .expect("No input dim found");
    ...
    while !boxes.is_empty() {
        ...
        boxes = boxes
            .iter()
            .filter(|box1| {
                intersection(&boxes[0].bbox, &box1.bbox) / union(&boxes[0].bbox, &box1.bbox) < 0.7
            })
            .cloned()
            .collect();
    }
